...
# Add the new table for NEMS L2 POD-bootstrapping
cat <<EOT >> "$output_file"
    </table>
    <br>
...
# Read the second input file and generate HTML rows for NEMS L2 POD-bootstrapping
while IFS=',' read -r component total_tests passed failed pass_percentage test_report report_status; do
    component=$(echo "$component" | xargs)
    total_tests=$(echo "$total_tests" | xargs)
    passed=$(echo "$passed" | xargs)
    failed=$(echo "$failed" | xargs)
    ...
    # Append HTML row for the current component in the new table
cat <<EOT >> "$output_file"
        <tr>
            <td>$component</td>
            <td>$total_tests</td>
            ...
            <td>$failed</td>
            <td class="$color_class">$pass_percentage</td>
            <td><a class="report-link" href="$test_report">View Report</a></td>
            <td>$report_status</td>
        </tr>
EOT

done < "$second_input_file"
