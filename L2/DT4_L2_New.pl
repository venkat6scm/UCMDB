...
<tr height = 100 bgcolor=#6699CC><th>COMPONENT NAME</th><th>TOTAL TEST</th><th>TEST PASSED</th><th>TEST FAILED</th><th>Test Report</th><th>Total Coverage</th></tr>";
...
#my $statusFile = $ARGV[0];
$ToEmail = "orga4-scrummasters\@radisys.com, orga4-managers\@radisys.com, orga4-architects\@radisys.com";
	...
	for(my $i =0;$i<=$#compilationName;$i++)
	{
		if (defined $buildStatus[$i])
		{
			if ($buildurl[$i]  > 0 || $buildurl[$i] =~/null/ )
     			{
#           	 		$failData=$failData."<tr><td><a href=\"$Build_duration[$i]\">$compilationName[$i]</a></td><td>$buildStatus[$i]</td><td>$buildtime[$i]</td><td  bgcolor=#FF4411>$buildurl[$i]</td></tr>";
