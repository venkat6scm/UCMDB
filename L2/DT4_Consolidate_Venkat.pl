...
$ccEmail = "DTA4_DevOps_RMQA1\@radisys.com";
# $ccEmail = "harikumar.nair\@radisys.com";
...
sub htmlForFail
{
	...
	for(my $i =0;$i<=$#compilationName;$i++)
	{
		if (defined $buildStatus[$i])
		{
			...
			{
				$failData=$failData."<tr><td><a href=\"$commitid[$i]\">$compilationName[$i]</a></td><td>$buildStatus[$i]</td><td>$buildtime[$i]</td><td>$buildurl[$i]</td><td bgcolor=#FFA500>$Build_duration[$i]</td><td><a href=\"$p2_Bul[$i]\">$p1_Bul[$i]</a></td><td>$p3_Bul[$i]</td></tr>";	
			}
			else
			{
