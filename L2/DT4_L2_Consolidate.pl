...
<tr height = 100 bgcolor=#6699CC><th>COMPONENT NAME</th><th>TOTAL TEST</th><th>TEST PASSED</th><th>TEST FAILED</th><th>Test Report</th></tr>";
...
$msg->attach(Type => 'html',
   Path => '/JenkinsBackup/DT_L2_new/L2_CI_Consolidate/voltha_report.html',
   Filename => 'voltha_report.html' ,
   Disposition => 'attachment'
);

$msg->attach(Type => 'html',
   Path => '/JenkinsBackup/DT_L2_new/L2_CI_Consolidate/pao_report.html',
   Filename => 'pao_report.html' ,
   Disposition => 'attachment'
);


$msg->attach(Type => 'html',
   Path => '/JenkinsBackup/DT_L2_new/L2_CI_Consolidate/ems_report.html',
   Filename => 'ems_report.html' ,
   Disposition => 'attachment'
);


$msg->attach(Type => 'html',
   Path => '/JenkinsBackup/DT_L2_new/L2_CI_Consolidate/nwm_pod_report.html',
   Filename => 'nwm_pod_report.html' ,
   Disposition => 'attachment'
);
...

{
                          ...
                          $failData=$failData."<tr><td><a href=\"$Build_duration[$i]\">$compilationName[$i]</a></td><td>$buildStatus[$i]</td><td bgcolor=#33EE11>$buildtime[$i]</td><td>$buildurl[$i]</td><td><a href=\"$p1_Bul[$i]\">$commitid[$i]</a></td></tr>";
...
$failData=$failData."<tr><td><a href=\"$Build_duration[$i]\">$compilationName[$i]</a></td><td>$buildStatus[$i]</td><td bgcolor=#33EE11>$buildtime[$i]</td><td bgcolor=#FF4411>$buildurl[$i]</td><td><a href=\"$p1_Bul[$i]\">$commitid[$i]</a></td></tr>";
...
$failData=$failData."<tr><td><a href=\"$Build_duration[$i]\">$compilationName[$i]</a></td><td>$buildStatus[$i]</td><td>$buildtime[$i]</td><td>$buildurl[$i]</td><td><a href=\"$p1_Bul[$i]\">$commitid[$i]</a></td></tr>";
			...
			#}		
                       ...
                       # }
                        ...
                        {
                               $failData=$failData."<tr><td><a href=\"$buildurl[$i]\">$compilationName[$i]</a></td><td bgcolor=#FFFF00>$buildStatus[$i]</td><td>$buildtime[$i]</td><td>$Build_duration[$i]</td><td>$commitid[$i]</td><td>$p1_Bul[$i]</td><td>$p2_Bul[$i]</td><td>$p3_Bul[$i]</td></tr>";
                        }
			if ($buildStatus[$i]  =~/Results/)
                        {


                           $failData=$failData."<tr><td bgcolor=#FFFF00>$compilationName[$i]</td><td bgcolor=#0000FF>$buildStatus[$i]</td><td></td><td></td><td>$buildurl[$i]</td><td>$Build_duration[$i]</td><td>$commitid[$i]</td></tr>";
