...
<tr height = 100 bgcolor=#6699CC><th>COMPONENT NAME</th><th>TOTAL TEST</th><th>TEST PASSED</th><th>TEST FAILED</th><th>TESTS PASSED PERCENTAGE</th> <th>Test Report</th><th>Report Status</th></tr>";
...
$ccEmail = "orgA4-Devops1\@radisys.com, holger.metschulat1\@telekom.de, Michael.Hoerth1\@telekom.de, a4-cicd-reports1\@telekom.de, v.santinelli1\@reply.de, DTA4_DevOps_RMQA1\@radisys.com,vuppara\@radisys.com";
...
sub htmlForFail
{
        ...
        for(my $i =0;$i<=$#compilationName;$i++)
        {
                ...
                {
                        if ($buildurl[$i]  > 0 || $buildurl[$i] =~/null/ )
                        {


                         $failData=$failData."<tr><td><a href=\"$Build_duration[$i]\">$compilationName[$i]</a></td><td>$buildStatus[$i]</td><td bgcolor=#33EE11>$buildtime[$i]</td><td bgcolor=#FF4411>$buildurl[$i]</td><td><td>$p3_Bul[$i]</td><a href=\"$p1_Bul[$i]\">$commitid[$i]</a></td><td>$p2_Bul[$i]</td></tr>";


                        }
                        else
                        {
                         $failData=$failData."<tr><td><a href=\"$Build_duration[$i]\">$compilationName[$i]</a></td><td>$buildStatus[$i]</td><td bgcolor=#33EE11>$buildtime[$i]</td><td>$buildurl[$i]</td><td><td>$p3_Bul[$i]</td><a href=\"$p1_Bul[$i]\">$commitid[$i]</a></td><td>$p2_Bul[$i]</td></tr>";
                        }
                        ...
                        if ($buildStatus[$i]  =~/Results/)
                        {
                           ...
                           $failData=$failData."<tr><td bgcolor=#FFFF00>$compilationName[$i]</td><td bgcolor=#0000FF>$buildStatus[$i]</td><td></td><td></td><td>$buildurl[$i]</td><td>$p3_Bul[$i]</td><td>$Build_duration[$i]</td><td>$commitid[$i]</td><td>$p1_Bul[$i]</td></tr>";
